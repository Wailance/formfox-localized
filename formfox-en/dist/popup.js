(() => {
  ...
  const elAutoConfirm = $("autoConfirm");
  ...
  async function setProfiles(nextProfiles) {
  ...
  async function deleteProfile(profileId) {
    const existing = await getProfiles();
    await setProfiles(existing.filter((p) => p.id !== profileId));
  }
  ...
  function renderDraft() {
    ...
    draftFields.forEach((f, idx) => elFields.appendChild(renderFieldRow(idx, f)));
  }
  ...
  async function onFillProfile(profile) {
    try {
      ...
      const msg = { type: "FILL_FORM", tabId: tab.id, profile };
  ...
  async function importJson(file) {
    try {
      ...
      const normalized = incoming
        ...
        .filter((p) => p.name.length > 0 && p.fields.length > 0);
