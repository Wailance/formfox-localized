...
function renderFieldRow(index: number, field: FormFoxField) {
  ...
  keyInput.placeholder = "Example: Tax ID";
  ...
  valInput.placeholder = "Example: 7707083893";
  ...
  btnDel.textContent = "Delete field";
...
function renderDraft() {
  ...
  if (draftFields.length === 0) {
    ...
    empty.textContent = "No fields yet. Add at least one.";
...
function renderProfilesList() {
  ...
  for (const p of profiles) {
    ...
    meta.textContent = `${p.fields.length} fields`;
...
async function onFillProfile(profile: FormFoxProfile) {
  try {
    ...
    if (!profile.autoConfirm) {
      ...
      const ok = confirm(`Fill profile "${profile.name}"?\nFields: ${preview}`);
      if (!ok) return;
    }
    ...
    if (!tab || typeof tab.id !== "number" || !tab.url) {
      setStatus("Failed to determine the active tab.", true);
      return;
    }
    ...
    setStatus("Autofill started. Dynamic forms may need a short delay.");
  } catch (err: any) {
...
async function onSaveProfile() {
  try {
    ...
    if (fields.length === 0) {
      setStatus("Add at least one field.", true);
      return;
    }
...
async function importJson(file: File) {
  try {
    ...
    if (normalized.length === 0) {
      setStatus("Import: no profiles found.", true);
      return;
    }

    if (!confirm(`Import ${normalized.length} profiles? The current list will be replaced.`)) return;
